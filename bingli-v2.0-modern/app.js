...
App({
  ...
  // 全局方法：显示成功提示 - 升级为现代化版本
  showSuccess(title) {
    try {
      // 尝试使用InteractionFeedback
      const InteractionFeedback = require('./utils/interaction-feedback');
      InteractionFeedback.showSuccess(title);
    } catch (e) {
      // 降级到系统Toast
      wx.showToast({
        title,
  ...
  // 全局方法：显示错误提示 - 升级为现代化版本
  showError(title) {
    try {
      // 尝试使用InteractionFeedback
      const InteractionFeedback = require('./utils/interaction-feedback');
      InteractionFeedback.showError(title);
    } catch (e) {
      // 降级到系统Toast
      wx.showToast({
        title,
  ...
  showInfo(title) {
    try {
      // 尝试使用InteractionFeedback
      const InteractionFeedback = require('./utils/interaction-feedback');
      InteractionFeedback.showInfo(title);
    } catch (e) {
      // 降级到系统Toast
      wx.showToast({
        title,
